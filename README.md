...
- `DUMA_ALIGNMENT` - This is an integer that specifies the alignment for any
  ...
  `-mips2` flag on _MIPS_-based systems such as those from *SGI*. For some
  architectures the default is defined to even more - x86\_64 uses alignment to
  16 bytes by default.
...
- `DUMA_ALLOW_MALLOC_0` - Memory allocation of size zero is *ANSI* conforming,
...
- `DUMA_MEMCPY_OVERLAP` - Set this shell environment variable to allow
  ...
  address. (workaround for *ARM* `memmove`/`memcpy` implementation).
