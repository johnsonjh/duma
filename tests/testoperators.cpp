...
public:
  ...
  void *operator new(DUMA_SIZE_T, const std::nothrow_t &, const char *,
                     int) throw();
  void operator delete(void *, const char *, int) throw();
  void operator delete(void *, const std::nothrow_t &, const char *,
                       int) throw();
  ...
  void operator delete[] ( void *, const std::nothrow_t &, const char *,
                           int ) throw();
#endif /* ifndef DUMA_NO_LEAKDETECTION */
...
void *
optest::operator new(DUMA_SIZE_T s) throw(std::bad_alloc)
{
  (void)s;
  return ( ::new optest );
}
...
void *
optest::operator new(DUMA_SIZE_T s, const std::nothrow_t &n) throw()
{
  (void)s;
  return ( ::new(n) optest );
}
...
void
optest::operator delete(void *p) throw()
{
  ::operator delete((optest *)p);
}

void
optest::operator delete(void *p, const std::nothrow_t &n) throw()
{
...
optest::operator new[] (DUMA_SIZE_T s)throw(std::bad_alloc)
{
  return ( ::new optest[s / sizeof ( optest )] ); // "s / sizeof()" not correct but
                                                  // works for this test
}
...
optest::operator new[] ( DUMA_SIZE_T s, const std::nothrow_t &n ) throw()
{
  return ( ::new(n) optest[s / sizeof ( optest )] ); // "s / sizeof()" not correct
                                                     // but works for this test
}
...
/* 3x : SINGLE OBJECT FORM - WITH DEBUG INFORMATION */
void *
optest::operator new(DUMA_SIZE_T s, const char *f,
                     int l) throw(std::bad_alloc)
{
  (void)s;
...
void *
optest::operator new(DUMA_SIZE_T s, const std::nothrow_t &n, const char *f,
                     int l) throw()
{
  (void)s;
...
void
optest::operator delete(void *p, const char *f, int l) throw()
{
  ::operator delete((optest *)p, f, l);
}

void
optest::operator delete(void *p, const std::nothrow_t &n, const char *f,
                        int l) throw()
{
...
{
  return ( ::new(f, l)
           optest[s / sizeof ( optest )] ); // "s / sizeof()" not correct but works for
                                            // this test
}

void *
optest::operator new[] ( DUMA_SIZE_T s, const std::nothrow_t &n,
                         const char *f,
                         int l ) throw()
{
  return ( ::new(n, f, l)
...
#else  /* ifndef DUMA_NO_LEAKDETECTION */
  s = new(std::nothrow) optest;
#endif /* ifndef DUMA_NO_LEAKDETECTION */
#include "../dumapp.h"
  delete s;
